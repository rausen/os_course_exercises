...
**区别：装载程序时只将当前需要的部分加载到内存中。在运行时如果发生缺页，就将内存中暂时不用的部分转移到外存中。**
...
**找到对应页面在外存中位置；将页面加载到内存中；修改页表项标志位；重新执行指令。**


2. 虚拟页式存储管理中有效存储访问时间是如何计算的？

**EAT=访存时间 $\times$ (1-p)+ 缺页处理时间 $\times$ p，其中p表示缺页率。**
